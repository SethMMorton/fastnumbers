...
__all__ = [
    "Error", "ThisTestFailed", "ResourceDenied", "import_module", "verbose",
    ...
    if sock.family == socket.AF_INET and sock.type == socket.SOCK_STREAM:
        if hasattr(socket, 'SO_REUSEADDR'):
            if sock.getsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR) == 1:
                raise ThisTestFailed("tests should never set the SO_REUSEADDR "   \
                                 "socket option on TCP/IP sockets!")
        if hasattr(socket, 'SO_REUSEPORT'):
            try:
                if sock.getsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT) == 1:
                    raise ThisTestFailed("tests should never set the SO_REUSEPORT "   \
                                     "socket option on TCP/IP sockets!")
            except OSError:
...
def open_urlresource(url, *args, **kw):
    ...
    if f is not None:
        return f
    raise ThisTestFailed('invalid resource %r' % fn)
...
def run_doctest(module, verbosity=None, optionflags=0):
    ...
    if f:
        raise ThisTestFailed("%d of %d doctests failed" % (f, t))
    if verbose:
